...
def addto_db(db_path, path, suffix_list, chunk_size=24 * 60 * 3, chunk_base=10000, movie_base=1000000):
    ...
    for isuffix, suffix in enumerate(suffix_list):
        ...
        assert isuffix == 0 or (prev_ichunk == ichunk and count == prev_count), '%d vs %d or %d vs %d' % (
        prev_ichunk, ichunk, count, prev_count)
        prev_ichunk = ichunk
        prev_count = count
...
#self.idx = [0]
    ...
    def next(self):
        ...
        if self.upsample > 1:
            left0 = np.zeros(
                (self.batch_size, self.data_shape[1] * self.upsample, self.data_shape[0] * self.upsample, 3),
                dtype=np.float32)
        else:
        ...
        right = np.zeros((self.batch_size, self.data_shape[1] * self.upsample, self.data_shape[0] * self.upsample, 3),
                         dtype=np.float32)
        if self.output_depth:
