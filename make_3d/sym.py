...
def make_upsample_sym(data, scale, fuse='sum', method='multi2'):
    ...
    pred1 = mx.symbol.Deconvolution(data=pred1, kernel=(1, 1), stride=(1, 1), pad=(0, 0), num_filter=33,
                                    no_bias=no_bias, workspace=workspace, name='deconv_pred1')
    scale *= 2
    pred2 = mx.symbol.Activation(data=pred2, act_type='relu')
    pred2 = mx.symbol.Deconvolution(data=pred2, kernel=(2 * scale, 2 * scale), stride=(scale, scale),
                                    pad=(scale / 2, scale / 2), num_filter=33, no_bias=no_bias, workspace=workspace,
                                    name='deconv_pred2')
    scale *= 2
    pred3 = mx.symbol.Activation(data=pred3, act_type='relu')
    pred3 = mx.symbol.Deconvolution(data=pred3, kernel=(2 * scale, 2 * scale), stride=(scale, scale),
                                    pad=(scale / 2, scale / 2), num_filter=33, no_bias=no_bias, workspace=workspace,
                                    name='deconv_pred3')
    scale *= 2
    pred4 = mx.symbol.Activation(data=pred4, act_type='relu')
    pred4 = mx.symbol.Deconvolution(data=pred4, kernel=(2 * scale, 2 * scale), stride=(scale, scale),
                                    pad=(scale / 2, scale / 2), num_filter=33, no_bias=no_bias, workspace=workspace,
                                    name='deconv_pred4')
    scale *= 2
...
def make_l1_sym(scale, fuse='sum', data_frames=1, flow_frames=1, get_softmax=False, method='multi2', upsample=1,
                tan=False):
